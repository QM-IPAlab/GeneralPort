...
class RavensDataset(Dataset):
    ...
    def __init__(self, path, cfg, n_demos=0, augment=False):
        ...
        if 'dataset' in cfg and 'in_shape' in cfg['dataset']:
            self.in_shape = cfg['dataset']['in_shape']
        else:
            print("Key 'dataset.in_shape' not found, using default value.")
            self.in_shape = (320, 160, 6)
        # if cfg['dataset']['in_shape']:
        #     self.in_shape = cfg['dataset']['in_shape']
        # else:
        #     self.in_shape = (320, 160, 6)
    ...
    def get_image(self, obs, cam_config=None):
        ...
        assert img.shape == tuple(self.in_shape), img.shape
        return img
