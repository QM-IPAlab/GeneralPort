...
class TransporterAgent(LightningModule):
    def __init__(self, name, cfg, train_ds, test_ds):
        ...
        # self._optimizers = {
        #     'attn': torch.optim.Adam(self.attention.parameters(), lr=self.cfg['train']['lr'], eps=self.cfg['train']['eps']),
        #     'trans': torch.optim.Adam(self.transport.parameters(), lr=self.cfg['train']['lr'], eps=self.cfg['train']['eps'])
        # }
        # pdb.set_trace()
        self._optimizers = {
            'attn': torch.optim.Adam(filter(lambda p: p.requires_grad, self.attention.parameters()), lr=self.cfg['train']['lr']),
            'trans': torch.optim.Adam(filter(lambda p: p.requires_grad, self.transport.parameters()), lr=self.cfg['train']['lr'])
        }
    ...
    def attn_criterion(self, backprop, compute_err, inp, out, p, theta):
        ...
        # Get loss.
        loss = self.cross_entropy_with_logits(out, label, reduction='mean')
        ...
        if backprop:
            ...
            clip_grad_value_(self.attention.parameters(), clip_value=0.001)
            attn_optim.step()
    ...
    def transport_criterion(self, backprop, compute_err, inp, output, p, q, theta):
        ...
        loss = self.cross_entropy_with_logits(output, label, reduction='mean')
        ...
        if backprop:
            ...
            clip_grad_value_(self.transport.parameters(), clip_value=0.001)
