...
class ConceptFusionAgent(TransporterAgent):
    ...
    def attn_forward(self, inp, softmax=True):
        ...
        sim = inp['sim']
        # pdb.set_trace()
        out = self.attention.forward(inp_img, lang_goal, sim, softmax=softmax)
        return out

    def attn_training_step(self, frame, sim, backprop=True, compute_err=False):
        inp_img = frame['img']
        p0, p0_theta = frame['p0'], frame['p0_theta']
        lang_goal = frame['lang_goal']
        # pdb.set_trace()

        inp = {'inp_img': inp_img, 'lang_goal': lang_goal, 'sim': sim}
    ...
    def trans_forward(self, inp, softmax=True):
        ...
        lang_goal = inp['lang_goal']
        sim = inp['sim']
        sim_crop = inp['sim_crop']

        # pdb.set_trace()
        out = self.transport.forward(inp_img, p0, lang_goal, sim_crop, sim, softmax=softmax)
        return out

    def transport_training_step(self, frame, sim1, sim2, backprop=True, compute_err=False):
        inp_img = frame['img']
        p0 = frame['p0']
        p1, p1_theta = frame['p1'], frame['p1_theta']
        lang_goal = frame['lang_goal']

        inp = {'inp_img': inp_img, 'p0': p0, 'lang_goal': lang_goal, 'sim_crop': sim1, 'sim': sim2}
        out = self.trans_forward(inp, softmax=False)
        err, loss = self.transport_criterion(backprop, compute_err, inp, out, p0, p1, p1_theta)
        return loss, err

    def predict_image_masks(self, frame):
        inp_img = frame['img']
        lang_goal = frame['lang_goal']

        sim1, sim2 = self.conceptfusion(inp_img, lang_goal, self.name, softmax=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # pdb.set_trace()
        sim1, sim2 = self.predict_image_masks(frame)

        # Get training losses.
        step = self.total_steps + 1
        loss0, err0 = self.attn_training_step(frame, sim1)
    
        if isinstance(self.transport, Attention):
            loss1, err1 = self.attn_training_step(frame, sim1)
        else:
            loss1, err1 = self.transport_training_step(frame, sim1, sim2)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(self.val_repeats):
            ...
            # pdb.set_trace()
            sim1, sim2 = self.predict_image_masks(frame)
            l0, err0 = self.attn_training_step(frame, sim1, backprop=False, compute_err=True)
            loss0 += l0
            if isinstance(self.transport, Attention):
                l1, err1 = self.attn_training_step(frame, sim1, backprop=False, compute_err=True)
                loss1 += l1
            else:
                l1, err1 = self.transport_training_step(frame, sim1, sim2, backprop=False, compute_err=True)
                loss1 += l1
    ...
    def test_step(self, batch, batch_idx):
        ...
        for i in range(self.val_repeats):
            frame, _ = batch
            sim1, sim2 = self.predict_image_masks(frame)
            l0, err0, out_attn = self.attn_training_step(frame, sim1, backprop=False, compute_err=True, return_output=True)
            loss0 += l0
            if isinstance(self.transport, Attention):
                l1, err1, out_attn = self.attn_training_step(frame, sim1, backprop=False, compute_err=True, return_output=True)
                loss1 += l1
            else:
                l1, err1, out_trans = self.transport_training_step(frame, sim2, backprop=False, compute_err=True, return_output=True)
                loss1 += l1
        loss0 /= self.val_repeats
        loss1 /= self.val_repeats
        val_total_loss = loss0 + loss1        
    ...
    def act(self, obs, info, goal=None):  # pylint: disable=unused-argument
        ...
        pick_inp = {'inp_img': img, 'lang_goal': lang_goal, 'sim': sim1}
        ...
        place_inp = {'inp_img': img, 'p0': p0_pix, 'lang_goal': lang_goal, 'sim': sim2, 'sim_crop': sim1}
...
class ConceptFusionSam2(ConceptFusionAgent):
    def __init__(self, name, cfg, train_ds, test_ds):
        super().__init__(name, cfg, train_ds, test_ds)
...
class ConceptFusionLarge(ConceptFusionAgent):
    def __init__(self, name, cfg, train_ds, test_ds):
        super().__init__(name, cfg, train_ds, test_ds)

    def _build_model(self):
        atten_stream_fcn = 'conceptfusion_clip_large'   # pick map conceptfusion_clip_large, conceptfusion_large_place, conceptfusion_large_kernel
        key_stream_fcn = 'conceptfusion_clip_large'     # place map
        query_stream_fcn = 'conceptfusion_large_kernel'   # crop
        self.attention = OneStreamAttenConceptFusion(
            stream_fcn=(atten_stream_fcn, None),
            in_shape=self.in_shape,
            n_rotations=1,
...
class Sam2Clip(ConceptFusionAgent):
    def __init__(self, name, cfg, train_ds, test_ds):
        super().__init__(name, cfg, train_ds, test_ds)

    def _build_model(self):
        atten_stream_fcn = 'sam2clip'   # pick map conceptfusion_clip_large, conceptfusion_large_place, conceptfusion_large_kernel
        key_stream_fcn = 'sam2clip'     # place map
        query_stream_fcn = 'sam2clip_kernel'   # crop
        self.attention = OneStreamAttenConceptFusion(
            stream_fcn=(atten_stream_fcn, None),
            in_shape=self.in_shape,
            n_rotations=1,
...
class Sam2Clip_wo(ConceptFusionAgent):
    def __init__(self, name, cfg, train_ds, test_ds):
        super().__init__(name, cfg, train_ds, test_ds)

    def _build_model(self):
        atten_stream_fcn = 'sam2clip_wo'   # pick map conceptfusion_clip_large, conceptfusion_large_place, conceptfusion_large_kernel
        key_stream_fcn = 'sam2clip_wo'     # place map
        query_stream_fcn = 'sam2clip_wo_kernel'   # crop
        self.attention = OneStreamAttenConceptFusion(
            stream_fcn=(atten_stream_fcn, None),
            in_shape=self.in_shape,
            n_rotations=1,
