...
class cocoop(conceptfusion):
    def __init__(self, cfg, device, preprocess, num_context_tokens=4, dynamic_context=True):
        ...
        self.num_context_tokens = 4
        self.dynamic_context = True

        # learnable context
        self.context_vectors = nn.Parameter(torch.randn(num_context_tokens, self.clip.ln_final.weight.shape[0]))
        
        # 
        if dynamic_context:
            self.context_mlp = nn.Linear(self.visual_encoder.output_dim, num_context_tokens * self.clip.ln_final.weight.shape[0])
    ...
    def similarity_map(self, input_color, input_depth, l1, l2, name):
        ...
        plt.imshow(out)
        plt.savefig("~/cliport/cliport/visualization/pretrain_image.png")
        # plt.figure(figsize=(20, 20))
        plt.imshow(img)
        show_anns(masks)
        plt.savefig("cliport/visualization/pretrain_masks.png") 
        ...
        if self.dynamic_context and outfeat is not None:
            context_dynamic = self.context_mlp(outfeat).view(-1, self.num_context_tokens, text_embedding.shape[-1])
            text_embedding = torch.cat([context_dynamic, text_embedding], dim=1)
        else:
            context_expanded = self.context_vectors.unsqueeze(0).expand(text_embedding.shape[0], -1, -1)
            text_embedding = torch.cat([context_expanded, text_embedding], dim=1)
        ...
        pdb.set_trace()
        plt.imshow(sim1)
        plt.savefig("/home/a/acw799/cliport/cliport/visualization/pretrain_sim1.png")
        plt.imshow(sim2)
        plt.savefig("/home/a/acw799/cliport/cliport/visualization/pretrain_sim2.png")
