...
class clipfit(conceptfusion):
    ...
    def similarity_map(self, input_color, input_depth, l1, l2, name):
        ...
        # extract all masks in the figure
        with torch.no_grad():
        ...
        l2 = 'A photo of ' + l2 + '.'
        if 'rope' in name:
            l1 = l1[:11] + 'both ends of rope.'
            l2 = l2[:11] + 'two corner, ' + l2[11:]
        ...
        for mask_data in masks_original:
            # pdb.set_trace()
            mask = mask_data["segmentation"]  # extract 2D masks (2D numpy )
            pixel_count = np.sum(mask > 0)    # sum up the number of non-zero pixels
            ...
            if 'packing-boxes-pairs' in name:
                if target_count == total_count:
                    continue
            if 'bowl' in name:    
                if target_count > 0.3 * total_count:# filter shadow masks
                    continue
            if 'pyramid' in name:
                if target_count == total_count:
                    continue
            ...
            if pixel_count >= 70: 
                ...
                # # add color for mask
                # mask_overlay[mask > 0] = [255, 0, 0]  
                
                # # mix initial image and mask
        ...
        plt.imshow(out)
        plt.savefig("~/cliport/cliport/visualization/failure_image.png")
        # plt.figure(figsize=(20, 20))
        plt.imshow(img)
        show_anns(masks)
        plt.savefig("cliport/visualization/failure_masks.png") 
        ...
        plt.imshow(sim1)
        plt.savefig("~/cliport/cliport/visualization/failure_sim1.png")
        plt.imshow(sim2)
        plt.savefig("~/cliport/cliport/visualization/failure_sim2.png")
