...
class CLIP(nn.Module):
    ...
    def encode_text_for_prompt_learning(self, x, tokens):
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
...
def load_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=False): # jit=True
    ...
    def patch_device(module):
        ...
        for graph in graphs:
            for node in graph.findAllNodes("prim::Constant"):
                if "value" in node.attributeNames() and str(node.attribute("value")).startswith("cuda"):
